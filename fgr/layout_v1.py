"""v1 -- the ORIGINAL layout generator (fixed grid + A* rip-up router). Kept as the historical
baseline / reference implementation; **v2** (:mod:`fgr.layout`) is the default and generally
faster and more reliable at scale -- see STATUS.md for the full v1-vs-v2 comparison. Select
this one explicitly via :mod:`fgr.generators` (``compile_graph(graph, "v1")``) or ``fgr compile
-g v1``.

Below is this module's original docstring, unchanged:
